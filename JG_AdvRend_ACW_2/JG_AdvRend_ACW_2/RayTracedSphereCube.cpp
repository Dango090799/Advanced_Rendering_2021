...
RayTracedSphereCube::RayTracedSphereCube(const shared_ptr<DeviceResources>& device)
	: _device(device), _loadingComplete(false), _indexCount(0)
{
	CreateDeviceDependentResources();
}

void RayTracedSphereCube::CreateDeviceDependentResources()
{
	auto loadVSTask = ReadDataAsync(L"VS_RayMarchObjects.cso");
	auto loadPSTask = ReadDataAsync(L"PS_RayTracedSphereCube.cso");
...
void RayTracedSphereCube::SetViewProjectionMatrixCB(XMMATRIX& view, XMMATRIX& projection)
{
...
void RayTracedSphereCube::SetInverseViewMatrixConstantBuffer(DirectX::XMMATRIX& inverseView)
{
	DirectX::XMStoreFloat4x4(&_inverseViewBufferData.inverseView, DirectX::XMMatrixTranspose(inverseView));
}

void RayTracedSphereCube::SetCameraPositionCB(XMFLOAT3& position)
{
	_cameraBufferData.position = position;
}

void RayTracedSphereCube::ReleaseDeviceDependentResources()
{
	...
	if (_inverseViewBuffer) _inverseViewBuffer.Reset();
}

void RayTracedSphereCube::Update(StepTimer const&)
{
	XMStoreFloat4x4(&_mvpBufferData.model, XMMatrixTranspose(XMMatrixIdentity()));
}

void RayTracedSphereCube::Render()
{
